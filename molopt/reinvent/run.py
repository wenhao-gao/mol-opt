...
class REINVENT(BaseOptimizer):
    ...
    def _optimize(self, oracle, config):
        ...
        path_here = os.path.dirname(os.path.realpath(__file__))
        restore_prior_from=os.path.join(path_here, 'data/reinvent.ckpt')

        if not os.path.exists(restore_prior_from):
            #### download pretrained model 
            url = "https://github.com/wenhao-gao/mol_opt/raw/main/main/reinvent/data/Prior.ckpt"
            response = requests.get(url, stream=True)
            total_size_in_bytes = int(response.headers.get("content-length", 0))
            block_size = 1024
            progress_bar = tqdm(total=total_size_in_bytes, unit="iB", unit_scale=True)
            # save_path = "reinvent.ckpt"
        ...
        if torch.cuda.is_available():
            Prior.rnn.load_state_dict(torch.load(os.path.join(path_here,'data/reinvent.ckpt')))
            Agent.rnn.load_state_dict(torch.load(restore_agent_from))
        else:
            Prior.rnn.load_state_dict(torch.load(os.path.join(path_here, 'data/reinvent.ckpt'), map_location=lambda storage, loc: storage))
